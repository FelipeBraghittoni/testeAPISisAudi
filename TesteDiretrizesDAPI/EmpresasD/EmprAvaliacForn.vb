Option Explicit On
Public Class EmprAvaliacForn
#Region "COM GUIDs"
    ' These  GUIDs provide the COM identity for this class 
    ' and its COM interfaces. If you change them, existing 
    ' clients will no longer be able to access the class.
    Public Const ClassId As String = "664501e9-7c28-42a3-989b-8f5537850305"
    Public Const InterfaceId As String = "6d50d7cc-1a12-42ed-b489-1c819d320305"
    Public Const EventsId As String = "2b89fcb2-ff08-4645-899c-5412e42c0305"
#End Region
...
#End Region
    ...
    Public Function dbConecta(abreDB As Short, tipo As Short, Optional vSelec As String = "") As Integer
        ...
        If tipo = 0 Then    'Aberto como OpenTable
            tAvaliacForn.Open("EmprAvaliacForn", Db, ADODB.CursorTypeEnum.adOpenDynamic, ADODB.LockTypeEnum.adLockOptimistic, )
...
EdbConecta:
        If Err.Number Then
            ...
            Else
                ...
                MsgBox("Classe EmprAvaliacForn - dbConecta" & vbCrLf & Err.Number & " - " & Err.Description)
            End If
        End If
...
EleSeq:
        If Err.Number Then
            leSeq = Err.Number
            MsgBox("Classe EmprAvaliacForn - leSeq" & vbCrLf & Err.Number & " - " & Err.Description)
        End If

    End Function

    Public Function localiza(empresa As Short, fornec As Short, data As Date, atualiz As Short) As Integer
        '* ****************************************
        ...
        vSelect = "SELECT * FROM EmprAvaliacForn WHERE idEmpresa = " & empresa
        vSelect = vSelect & " AND idFornecedor = " & fornec
        vSelect = vSelect & " AND Convert(datetime, dtAvaliacao, 112) = '" & Format(data, "yyyymmdd") & "'"
        tAvaliacForn.Open(vSelect, Db, ADODB.CursorTypeEnum.adOpenDynamic)

        localiza = 0    'ReturnCode se não houver nenhum problema

        'Não encontrou:
        If tAvaliacForn.EOF Then
            localiza = 1054
...
Elocaliza:
        If Err.Number Then
            ...
            Else
                localiza = Err.Number
                MsgBox("Classe EmprAvaliacForn - localiza" & vbCrLf & Err.Number & " - " & Err.Description)
            End If
        End If
    ...
    Public Function inclui(avaliacao As String) As Integer
        ...
        tAvaliacForn.AddNew()
        tAvaliacForn.Fields("idEmpresa").Value = idEmpresa
        tAvaliacForn.Fields("idFornecedor").Value = idFornecedor
        tAvaliacForn.Fields("nomeAvaliador").Value = nomeAvaliador
        tAvaliacForn.Fields("cargoAvaliador").Value = cargoAvaliador
        tAvaliacForn.Fields("dtAvaliacao").Value = dtAvaliacao
        tAvaliacForn.Fields("txtAvaliacao").Value = IIf(avaliacao = "", " ", avaliacao)
        tAvaliacForn.Update()
        inclui = 0
...
Einclui:
        If Err.Number Then
            inclui = Err.Number
            MsgBox("Classe EmprAvaliacForn - inclui" & vbCrLf & Err.Number & " - " & Err.Description)
        End If
    ...
    Public Function altera(avaliacao As String) As Integer
        ...
        tAvaliacForn(2).Value = nomeAvaliador
        tAvaliacForn(3).Value = cargoAvaliador
        tAvaliacForn(5).Value = IIf(avaliacao = "", " ", avaliacao)
        tAvaliacForn.Update()
        altera = 0
...
Ealtera:
        If Err.Number Then
            altera = Err.Number
            MsgBox("Classe EmprAvaliacForn - altera" & vbCrLf & Err.Number & " - " & Err.Description)
        End If
    ...
    Public Function elimina(empresa As Short, fornec As Short, data As Date) As Integer
        '* ****************************************
        ...
        vSelec = "DELETE FROM EmprAvaliacForn WHERE "
        vSelec = vSelec & " idEmpresa = " & empresa
        vSelec = vSelec & " AND idFornecedor = " & fornec
...
Eelimina:
        If Err.Number Then
            elimina = Err.Number
            MsgBox("Classe EmprAvaliacForn - elimina" & vbCrLf & Err.Number & " - " & Err.Description)
        End If
